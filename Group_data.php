...
class Group_data extends CI_Controller{
 ...
 function __construct() {
    ...
    public function group_tabdetails() //tabedetails
    {
        ...
        elseif ($header_name == 'event') 
         {
            // echo 'Keep going. Good luck!';
            $header_name = $request->header_name;
            $group_id = $request->group_id;
            // print_r($group_id);die();
            $where = array('event.group_id' => $group_id);
                //CONCAT('w3resource','.','com')
            $res = $this->Core_Model->joindataResult($place1 ='user.user_id', $place2 = 'event.user_id',$where,'event.id,event.group_id,event.user_id,event.title,event.location,event.description,user.image','user','event',$order='');
            // print_r($res);die();
            $userdata = [];
            foreach ($res as $total) 
            {
                $userdata[] = array('id' => $total['id'],'group_id' => $total['group_id'],'user_id' => $total['user_id'],'title' => $total['title'],'location' => $total['location'],'description' => $total['description'],'image' => base_url('assets/img/').$total['image']);
                // print_r($userdata);die();
            }
            if (empty($userdata)) 
            {
            ...
            } else
             {
                ...
                $where1 = $res[0]['group_id'];
                // print_r($where1);die();
                $res1 = $this->Core_Model->Countevent_group($where1);
                $this->res->success = true;
                $this->res->data = $userdata;
                $this->res->events = $res1;
            }
                $this->_output();
                exit();
        }
